...
class CityTabListView: UIView {
    ...
    let didSelectTabObserver = PublishRelay<CityTab>()
    ...
    private func bind() {
        let viewAll = viewAllButton.rx.tap.asObservable().map{ CityTab.viewAll }
        let culturalheritage = culturalheritageButton.rx.tap.asObservable().map { CityTab.culturalheritage }
        let park = parkButton.rx.tap.asObservable().map { CityTab.park }
        let tourismSpecialZone = tourismSpecialZoneButton.rx.tap.asObservable().map { CityTab.specialTouristZone }
        let centralBusinessDistrict = centralBusinessDistrictButton.rx.tap.asObservable().map { CityTab.centralBusinessDistrict }
        let denselyPopulatedArea = denselyPopulatedAreaButton.rx.tap.asObservable().map { CityTab.denselyPopulatedArea }
        
        let allButtonTaps = Observable.merge(viewAll, culturalheritage, park, tourismSpecialZone, centralBusinessDistrict, denselyPopulatedArea)
        
        allButtonTaps
            .subscribe(onNext: { [weak self] tab in
                self?.didSelectTabObserver.accept(tab)
            })
